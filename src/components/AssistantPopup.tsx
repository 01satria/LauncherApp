...
// Chat cache — capped at 200 messages to prevent unbounded growth
const MAX_MESSAGES = 200;
let _chatCache: Message[] | null = null;
const initChat = (): Message[] => { if (_chatCache !== null) return _chatCache; _chatCache = []; return _chatCache; };
const saveChat = (msgs: Message[]) => { _chatCache = msgs.length > MAX_MESSAGES ? msgs.slice(-MAX_MESSAGES) : msgs; };

// ── Fuzzy matching — reuse single buffer ──────────────────────────────────────
const RE_PUNCT = /[''`.,!?]/g;
const RE_SPACE = /\s+/g;
const norm = (s: string) => s.toLowerCase().replace(RE_PUNCT, '').replace(RE_SPACE, ' ').trim();

// Reusable dp buffer — avoids Uint16Array allocation per call
let _dpBuf: Uint16Array | null = null;
const lev = (a: string, b: string): number => {
  const m = a.length, n = b.length;
  const size = (m + 1) * (n + 1);
  if (!_dpBuf || _dpBuf.length < size) _dpBuf = new Uint16Array(size);
  const dp = _dpBuf;
  for (let i = 0; i <= m; i++) dp[i * (n + 1)] = i;
  for (let j = 0; j <= n; j++) dp[j] = j;
  for (let i = 1; i <= m; i++)
    for (let j = 1; j <= n; j++)
      dp[i*(n+1)+j] = a[i-1]===b[j-1] ? dp[(i-1)*(n+1)+(j-1)]
        : 1 + Math.min(dp[(i-1)*(n+1)+j], dp[i*(n+1)+(j-1)], dp[(i-1)*(n+1)+(j-1)]);
  return dp[m*(n+1)+n];
};
...
const getReply = (input: string, userName: string, assistantName: string): string => {
  const t = norm(input);
  if (hasKeyword(t, ['hai','hi','halo','hello','hey','sup','yo','howdy'], 0.82)) return `Hey ${userName}! 👋 What can I do for you?`;
  if (hasKeyword(t, ['apa kabar','how are you','gimana kabar','hows it going','how r u','kabar kamu'])) return `I'm doing great, ${userName}! 😊 How about you?`;
  if (hasKeyword(t, ['siapa kamu','who are you','nama kamu','namamu','your name','siapa lo','who r u'])) return `I'm ${assistantName}, your personal assistant! 🤖`;
  if (hasKeyword(t, ['makasih','thanks','thank you','thx','tengkyu','ty','terima kasih','thankyou','tq'])) return `You're welcome, ${userName}! 😊`;
  if (hasKeyword(t, ['bisa apa','what can you do','kamu bisa apa','capabilities','fitur','apa kemampuan'])) return `I can chat with you, tell you the time, and help with reminders! 💬`;
  if (hasKeyword(t, ['jam berapa','what time','whats time','what is time','current time','waktu sekarang','jam sekarang','time now','pukul berapa'], 0.72)) return `It's ${getTimeStr()} right now, ${userName}! ⏰`;
  if (hasKeyword(t, ['bored','bosan','gabut','nothing to do','iseng','boring'])) return `Try opening your favorite app! 📱 Or just keep chatting with me 😄`;
  if (hasKeyword(t, ['capek','lelah','tired','exhausted','ngantuk','sleepy','kecapekan'])) return `Take a break, ${userName}! 😴 Your health matters.`;
  if (hasKeyword(t, ['lapar','makan','hungry','food','eat','dinner','lunch','breakfast','belum makan','laper'])) return `Hungry? Don't skip your meals, ${userName}! 🍔`;
  if (hasKeyword(t, ['bye','dadah','sampai jumpa','see you','goodbye','later','ciao','selamat tinggal','good bye'])) return `Bye ${userName}! 👋 See you later~`;
  if (/^(oke|ok|sip|siap|got it|alright|sure|noted|oke deh|oke siap)$/.test(t)) return `Got it! 👍`;
  if (hasKeyword(t, ['tolong','help','bantuan','assist','minta tolong','please help','butuh bantuan'])) return `I'm right here, ${userName}! Tell me what's on your mind. 😊`;
  if (hasKeyword(t, ['good morning','selamat pagi','pagi pagi','morning','gm'])) return `Good morning, ${userName}! ☀️ Hope you have a great day!`;
  if (hasKeyword(t, ['good night','selamat malam','good evening','malam','gn','goodnight'])) return `Good night, ${userName}! 🌙 Sweet dreams!`;
  if (hasKeyword(t, ['i love you','love u','aku suka kamu','sayang kamu','luv u','i luv you'])) return `Aww, I appreciate that, ${userName}! 🥰`;
  if (hasKeyword(t, ['miss you','kangen','i miss','miss u','kangen kamu'])) return `I'm always right here for you, ${userName}! 💚`;
  if (hasKeyword(t, ['joke','jokes','lucu','funny','cerita lucu','buat lelucon','tell me a joke'])) return `Why don't scientists trust atoms? Because they make up everything! 😄`;
  if (hasKeyword(t, ['weather','cuaca','hujan','rain','panas','hot','cold','dingin','mendung'])) return `I can't check the weather, but stay safe out there, ${userName}! 🌤️`;
  const defaults = [
...
export const AnimatedToggle = memo(({ value, onValueChange, activeColor = '#27ae60' }: ToggleProps) => {
  ...
  const panResponder = useRef(PanResponder.create({
    ...
    onPanResponderMove: (_: GestureResponderEvent, gs) => {
      ...
      posX.setValue(next); bgAnim.setValue((next - 2) / MAX_X);
    },
...
// ─── Typing Indicator — stops animation when app inactive ────────────────────
  ...
  useEffect(() => {
    const startLoop = () => {
      loopRef.current = Animated.loop(
        Animated.sequence([
          Animated.stagger(160, dots.map(d => Animated.sequence([
    ...
    const sub = AppState.addEventListener('change', (s: AppStateStatus) => {
      if (s === 'active') { loopRef.current?.stop(); dots.forEach(d => d.setValue(0)); startLoop(); }
      else { loopRef.current?.stop(); dots.forEach(d => d.setValue(0)); }
    });
    return () => { loopRef.current?.stop(); dots.forEach(d => d.stopAnimation()); sub.remove(); };
  }, []);
...
const TypingBubble = memo(() => (
  <View style={[bubbleStyles.row, bubbleStyles.rowLeft]}>
    <View style={[bubbleStyles.bubble, bubbleStyles.aiBubble]}><TypingIndicator /></View>
  </View>
));
...
const AssistantPopup = memo(({ onClose, userName, assistantName, avatarSource }: AssistantPopupProps) => {
  ...
  const sendMessage = useCallback(() => {
    ...
    replyTimer.current = setTimeout(() => {
      const reply: Message = { id:makeId(), text:getReply(text, userName, assistantName), from:'assistant', time:getTimeStr() };
      setIsTyping(false);
  ...
  const listFooter    = isTyping ? <TypingBubble /> : null;
